...
namespace DOL.GS.Scripts
{
	public class PrisonGardian : GameNPC
	{
		...
		public override bool Interact(GamePlayer player)
		{
			...
			if (Activate)
			{
                if (player.Client.Account.PrivLevel >= (int)ePrivLevel.GM)
					player.Out.SendMessage("(Menu GM) [Desactivation Gardien]", eChatType.CT_System, eChatLoc.CL_PopupWindow);
				...
				if (objs.Count() > 0)
				{
					player.Out.SendMessage("Pour quelques pieces d'or, je peux liberer les prisoniers ...\n Voici la liste des prisoniers:", eChatType.CT_System, eChatLoc.CL_PopupWindow);

					int nb_prisoners = 0;
                    foreach (KeyValuePair<GamePlayer, Prisoner> kp in objs)
                    {
                        string textePrisonier = "[" + kp.Key.Name + "] (Cout: " + kp.Value.Cost + " Or)";
                        player.Out.SendMessage(textePrisonier, eChatType.CT_System, eChatLoc.CL_PopupWindow);
                        nb_prisoners++;
                    }
				    if (nb_prisoners == 0)
						player.Out.SendMessage("Desole, il n'y a aucun prisonier dans ce monde actuellement.", eChatType.CT_System, eChatLoc.CL_PopupWindow);
				}
				else
		...
		public override bool WhisperReceive(GameLiving source, string text)
		{
		    ...
		    player.Out.SendMessage("Vous venez de liberer " + text + " pour " + PrixTotal + " or !", eChatType.CT_System, eChatLoc.CL_PopupWindow);
		    return true;
		}
	}
}