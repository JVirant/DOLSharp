...
namespace DOL.GS.PacketHandler
{
	...
	public class PacketLib1124 : AbstractPacketLib, IPacketLib
	{
		...
		protected virtual void SendInventoryItemsPartialUpdate(IDictionary<int, InventoryItem> items, eInventoryWindowType windowType)
		{
			...
			{
				GameVault houseVault = _gameClient.Player.ActiveInventoryObject as GameVault;
				pak.WriteByte((byte)(items.Count));
				pak.WriteByte(0x00); // new in 189b+, show shield in left hand
				pak.WriteByte((byte)((_gameClient.Player.IsCloakInvisible ? 0x01 : 0x00) | (_gameClient.Player.IsHelmInvisible ? 0x02 : 0x00))); // new in 189b+, cloack/helm visibility
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.InventoryUpdate)))
			{
				...
				if (slots != null)
				{
					foreach (int updatedSlot in slots)
					{
						...
						WriteItemData(pak, _gameClient.Player.Inventory.GetItem((eInventorySlot)(updatedSlot)));
		...
		public virtual void SendMasterLevelWindow(byte ml)
		{
			...
			byte mlToSend = (byte)(ml == 0 ? (_gameClient.Player.MLLevel == 0 ? 1 : _gameClient.Player.MLLevel) : ml);
			...
			{
				mlXPPercent = 100.0 * _gameClient.Player.MLExperience / _gameClient.Player.GetMLExperienceForLevel(_gameClient.Player.MLLevel + 1);
				if (_gameClient.Player.GetStepCountForML((byte)(_gameClient.Player.MLLevel + 1)) > 0)
				{
					mlStepPercent = 100.0 * _gameClient.Player.GetCountMLStepsCompleted((byte)(_gameClient.Player.MLLevel + 1)) / _gameClient.Player.GetStepCountForML((byte)(_gameClient.Player.MLLevel + 1));
				}
			}
			else
			{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut((byte)eServerPackets.MasterLevelWindow))
			{
				...
				for (int i = 1; i < 11; i++)
				{
					string description = _gameClient.Player.GetMLStepDescription(mlToSend, i);
		...
		public virtual void SendPlayerPositionAndObjectID()
		{
			...
			{
				pak.WriteFloatLowEndian(_gameClient.Player.X);
				pak.WriteFloatLowEndian(_gameClient.Player.Y);
				pak.WriteFloatLowEndian(_gameClient.Player.Z);
				...
				pak.WriteByte((byte)(flags));
				if (_gameClient.Player.CurrentRegion.IsHousing)
				{
		...
		public virtual void SendRegions()
		{
			if (_gameClient.Player != null)
			{
				if (!_gameClient.Socket.Connected)
					return;
				Region region = WorldMgr.GetRegion(_gameClient.Player.CurrentRegionID);
				if (region == null)
					return;
		...
		public virtual void SendStatusUpdate(byte sittingFlag)
		{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterStatusUpdate)))
			{
				...
				pak.WriteByte(0);// unk
				pak.WriteShort((ushort)_gameClient.Player.MaxMana);
				pak.WriteShort((ushort)_gameClient.Player.MaxEndurance);
				pak.WriteShort((ushort)_gameClient.Player.MaxConcentration);
				pak.WriteShort((ushort)_gameClient.Player.MaxHealth);
				pak.WriteShort((ushort)_gameClient.Player.Health);
				pak.WriteShort((ushort)_gameClient.Player.Endurance);
				pak.WriteShort((ushort)_gameClient.Player.Mana);
				pak.WriteShort((ushort)_gameClient.Player.Concentration);
				SendTCP(pak);
			}
		}
		...
		public virtual void SendUpdatePoints()
		{
			...
			using (GSTCPPacketOut pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CharacterPointsUpdate)))
			{
				...
				pak.WriteLongLowEndian((ulong)_gameClient.Player.Experience);
				pak.WriteLongLowEndian((ulong)_gameClient.Player.ExperienceForNextLevel);
		...
		public virtual void SendUDPInitReply()
		{
			using (var pak = new GSUDPPacketOut(GetPacketCode(eServerPackets.UDPInitReply)))
			{
				...
				if (!_gameClient.Socket.Connected) // || !_gameClient.UsingRC4) // not using RC4, wont accept UDP packets anyway.)
				{
					return;
				}
		...
		public virtual void SendVersionAndCryptKey()
		{
			...
			using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.CryptKey)))
			{
				...
				pak.WriteByte((byte)_gameClient.MinorRev[0]);

				// Build number
				pak.WriteByte(_gameClient.MajorBuild); // last seen : 0x44 0x05
				pak.WriteByte(_gameClient.MinorBuild);
				SendTCP(pak);
			}
		}
		...
		public virtual void SendUpdateWeaponAndArmorStats()
		{
			...
			using (var pak = new GSTCPPacketOut(GetPacketCode(eServerPackets.VariousUpdate)))
			{
				...
				// weapondamage
				var wd = (int)(_gameClient.Player.WeaponDamage(_gameClient.Player.AttackWeapon) * 100.0);
