...
namespace DOL.GS.PacketHandler
{
	...
	public class PacketLib1125 : PacketLib1124
	{
		...
		public override void SendVersionAndCryptKey()
		{
			...
			{
				pak.WritePascalStringIntLE((((int)_gameClient.Version) / 1000) + "." + (((int)_gameClient.Version) - 1000) + _gameClient.MinorRev);
				//// Same as the trailing two bytes sent in first client to server packet
				pak.WriteByte(_gameClient.MajorBuild); // last seen : 0x2A 0x07
				...
				else
				{
					...
					for (int i = firstSlot; i < (firstSlot + 10); i++)
					{
						...
						else
						{
							...
							if (region != null)
							{
								locationDescription = region.GetTranslatedSpotDescription(_gameClient, c.Xpos, c.Ypos, c.Zpos);
							}
		...
		public override void SendUDPInitReply()
		{
			using (var pak = new GSUDPPacketOut(GetPacketCode(eServerPackets.UDPInitReply)))
			{
				...
				if (!_gameClient.Socket.Connected) // not using RC4, wont accept UDP packets anyway.
				{
					return;
				}
